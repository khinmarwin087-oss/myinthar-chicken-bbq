...
export default function OrderHistory() {
    ...
    useEffect(() => {
        const unsubscribeAuth = auth.onAuthStateChanged((u) => {
            setUser(u);
            if (u) {
                // Real-time tracking အတွက် onSnapshot သုံးပါတယ်
                const q = query(collection(db, "orders"), where("email", "==", u.email));
                const unsubscribeOrders = onSnapshot(q, (snapshot) => {
                    const list = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
                    
                    // Index error မတက်အောင် client-side မှာပဲ sorting လုပ်ပေးပါတယ်
                    list.sort((a, b) => {
                    ...
                    // လက်ရှိကြည့်နေတဲ့ အော်ဒါ status ပြောင်းရင် detail view မှာပါ ပြောင်းအောင် update လုပ်ပါတယ်
                    if (selectedOrder) {
        ...
        <div style={{ background: '#F8F9FA', minHeight: '100vh', padding: '20px' }}>
            ...
            {!selectedOrder ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                    {orders.length === 0 ? (
                        <p style={{ textAlign: 'center', color: '#999' }}>မှာယူထားသော မှတ်တမ်းမရှိသေးပါ</p>
                    ) : (
                        orders.map(order => (
                            <div 
                                key={order.id} 
                ...
                <div style={{ background: '#fff', borderRadius: '20px', padding: '25px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
                    <div style={{ textAlign: 'center', marginBottom: '30px' }}>
                        <h3 style={{ margin: '0 0 15px 0' }}>Order {selectedOrder.status === 'Ready' ? 'Completed' : 'Status'}</h3>
                        
                        {/* Tracking Progress Bar */}
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0', marginBottom: '10px' }}>
                            <div style={{ width: '25px', height: '25px', borderRadius: '50%', background: '#007AFF', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', zIndex: 2 }}>✓</div>
                            <div style={{ width: '60px', height: '3px', background: getStatusStyle(selectedOrder.status).progress >= 2 ? '#007AFF' : '#eee' }}></div>
                            <div style={{ width: '25px', height: '25px', borderRadius: '50%', background: getStatusStyle(selectedOrder.status).progress >= 2 ? '#007AFF' : '#fff', color: getStatusStyle(selectedOrder.status).progress >= 2 ? '#fff' : '#ccc', border: '2px solid', borderColor: getStatusStyle(selectedOrder.status).progress >= 2 ? '#007AFF' : '#eee', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', zIndex: 2 }}>{getStatusStyle(selectedOrder.status).progress >= 2 ? '✓' : '2'}</div>
                            <div style={{ width: '60px', height: '3px', background: getStatusStyle(selectedOrder.status).progress >= 3 ? '#007AFF' : '#eee' }}></div>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', width: '210px', margin: '0 auto', fontSize: '10px', color: '#999', fontWeight: 'bold' }}>
                            <span>PLACED</span>
                            <span>COOKING</span>
                            <span>READY</span>
                        </div>
                    ...
                    <div style={{ marginBottom: '20px' }}>
                        <h4 style={{ margin: '0 0 10px 0', fontSize: '15px' }}>Items</h4>
                    ...
                    <div style={{ borderTop: '2px dashed #eee', paddingTop: '15px' }}>
                        ...
                        <div style={{ fontSize: '13px', color: '#666', background: '#F9F9F9', padding: '15px', borderRadius: '12px' }}>
                            <div>Name: {selectedOrder.name}</div>
                            <div>Phone: {selectedOrder.phone}</div>
                            {selectedOrder.note && <div style={{ marginTop: '5px' }}>Note: {selectedOrder.note}</div>}
                        </div>
                    </div>

                    <div style={{ display: 'flex', gap: '10px', marginTop: '25px' }}>
                        <button onClick={() => setSelectedOrder(null)} style={{ flex: 1, padding: '15px', borderRadius: '12px', border: '1px solid #eee', background: '#fff', fontWeight: 'bold' }}>Back</button>
                        <a href={`tel:${selectedOrder.phone}`} style={{ flex: 1, padding: '15px', borderRadius: '12px', background: '#007AFF', color: '#fff', fontWeight: 'bold', textAlign: 'center', textDecoration: 'none' }}>Call Shop</a>
                    </div>
                </div>
            )}
        </div>
